...
banner="
		...
		${BWhite}Mobile Application Vulnerability Scanner${Off} |
"
...
if ! command -v apkinfo &> /dev/null; then
...
if [ -f "$APK" ]; then 
	...
	d2j-dex2jar $APK -o apk.jar > /dev/null 2>&1	# create jar file from apk
		...
		# Check Snapshots Allowed
echo -ne " Check ${BWhite}Snapshots Allowed${Off}: "
...
if [ -z "$check" ]; then
    ...
    if [ -n "$EXPLOIT" ]; then
        ...
        echo -e "${ANSWER}will have been created. Verify the image is not blank"
        echo -e "${ANSWER}by accessing it. Commands to access images:"
        # Modify the following commands as needed
        echo -e "${ANSWER}${BCyan}adb shell \"su -c 'cp -r /data/system/recent_images /sdcard/'\"${Off}"
        echo -e "${ANSWER}adb pull /sdcard/recent_images" 
        echo -e "${ANSWER}adb shell \"su -c 'rm -r /sdcard/recent_images'\"${Off}"
        echo -e "${ANSWER}${BCyan} or "
        echo -e "${ANSWER}${BCyan}adb shell \"su -c 'cp -r /data/system_ce/0/snapshots /sdcard/'\"${Off}"
        echo -e "${ANSWER}adb pull /sdcard/snapshots" 
        echo -e "${ANSWER}adb shell \"su -c 'rm -r /sdcard/snapshots'\"${Off}"
    fi
fi

# Clean up the jar file if it exists
if [ -f "apk.jar" ]; then
    rm apk.jar
fi
